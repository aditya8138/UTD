...
1. Source send __Reserve message__ to destination along the route, giving
   [_**flowspec**_](#flow-specification) (burstiness, rate, desired delay) and reserving resources
   along the way.
...
<center>
![](http://latex.codecogs.com/svg.latex?\left(\sum{i,j}:A_{f}^i\in[t,t^{\prime}]\land{D_{f}^i}\leq{t^{\prime}}:L_{f}^i\right\)\leq{\(t^{\prime}-t\)\times{C_{out}_{}})
</center>
...
<center>
![](http://latex.codecogs.com/svg.latex?E_f^i\leq{T_f^i}+\frac{L_{max}}{C_{out}}),
</center>
__provided,__
<center>
![](http://latex.codecogs.com/svg.latex?\sum_{g=1}^N=R_g\leq{C_{out}})
</center>
...
has bounded appetite, where
![](http://latex.codecogs.com/svg.latex?T_f^i=D_f^i).
...
![](http://latex.codecogs.com/svg.latex?t^{\prime}) add to no more than
<center>
![](http://latex.codecogs.com/svg.latex?\left\(t^{\prime}-t\right\)\times{C_{out}_{\phantom}})
</center>
