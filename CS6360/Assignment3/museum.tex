...
\begin{tikzpicture}[node distance=6em, every edge/.style={link}]
    ...
    \node[attribute] (idno) [left=4em of art_objects] {IDNo} edge (art_objects);
    ...
    \node[relationship] (create_by) [right=5em of art_objects] {Create By} edge (art_objects);

    \node[entity] (artist) [right=5em of create_by] {Artist} edge (create_by);
    ...
    \node[specialization] (type) [below = 5em of art_objects] {d} edge (art_objects);
    \node[entity] (sculpture) [below =5em of type] {SCULPTURE/STATUE} edge node {$\bigcup$}(type);
    \node[attribute] (sculpture_material) [below left=6em and 1.5em of sculpture] {Material} edge (sculpture);
    \node[attribute] (sculpture_height) [below left= 5em and -4.5em of sculpture] {Height} edge (sculpture);
    \node[attribute] (sculpture_weight) [below right= 5em and -4.5em of sculpture] {Weight} edge (sculpture);
    \node[attribute] (sculpture_style) [below right= 3em and -0.5em of sculpture] {Style} edge (sculpture);

    \node[entity] (painting) [left=4em of sculpture] {PAINTING} edge node [rotate=-45] {$\bigcup$}(type);
    ...
    \node[entity] (other) [right=4em of sculpture] {OTHER} edge node [rotate=45] {$\bigcup$} (type);
    \node[attribute] (other_type) [below of=other] {Type} edge (other);
    \node[attribute] (other_style) [below right of=other] {Style} edge (other);

    \node[specialization] (belong_type) [below right= 8em and 25em of type] {d} edge (art_objects);


    \node[entity] (permanent_collection) [below right=12em and 3em of belong_type] {PERMANENT COLLECTION} edge node {$\bigcup$} (belong_type);
    ...
    \node[specialization] (display_status) [below=8em of permanent_collection] {d} edge (permanent_collection);
    \node[entity] (on_display) [below left=3em and 2em of display_status] {On Display} edge node {$\bigcup$} (display_status);
    \node[entity] (stored) [below right=3em and 2em of display_status] {Stored} edge node {$\bigcup$} (display_status);

    \node[relationship] (display_on) [left=10em of on_display] {Display On} edge (on_display);
    \node[entity] (exhibitions) [left=6em of display_on] {EXHIBITIONS} edge (display_on);
    ...
    \node[entity] (borrowed_collection) [left=23em of permanent_collection] {BORROWED} edge node [rotate=70] {$\bigcup$} (belong_type);
