...
\begin{tikzpicture}[node distance=6em, every edge/.style={link}]
    ...
    \node[entity] (artist) [right=5em of create_by] {Artist} edge node[auto,swap, near end] {M} (create_by);
    ...
    \node[entity] (permanent_collection) [below right=12em and 3em of belong_type] {PERMANENT COLLECTION} edge node [rotate=35] {$\bigcup$} (belong_type);
    ...
    \node[entity] (borrowed_collection) [left=23em of permanent_collection] {BORROWED} edge node [rotate=-56] {$\bigcup$} (belong_type);
